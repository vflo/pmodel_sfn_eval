...
model_numerical <- function(tc, ppfd, vpd, co2, elv, fapar, kphio, psi_soil, 
  ...
  #Phydro/phydro_cmax/phydro_sperry/phydro_sperry
...
fn_calib <- function(par, data_temp, stomatal_model){
  ...
  calib_sperry = tibble(var = data_temp$Predawn.LWP..MPa.) %>% 
  ...
  data_temp1 <- data_temp %>%
    left_join(calib_sperry %>% dplyr::rename('Predawn.LWP..MPa.'=var),
              by = 'Predawn.LWP..MPa.')%>% 
...
calibrate_sperry_parameters <- function(data_temp, cpar, stomatal_model){
...
cpar_sperry <- dat1 %>% 
  ...
  group_by(Species) %>% 
  dplyr::group_split()%>% 
  purrr::map_df(~calibrate_sperry_parameters(., cpar,"phydro_sperry")) %>% 
  dplyr::bind_cols()


save(cpar_sperry, file = "DATA/cpar_sperry.RData")
