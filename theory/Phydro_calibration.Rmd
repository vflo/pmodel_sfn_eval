...
load("../DATA/cpar_wang.RData")
# load("../DATA/cpar_sox.RData")
# load("../DATA/cpar_sperry.RData")
# cpar_wang <- cpar_sperry
# load("../DATA/cpar_phydro_victor.RData")
# cpar_wang <- cpar_phydro
...
plot_all = function(df_w_vol, varname, species, b,a){
  ...
  #Wang
  gx_wang = log(df_w_vol$wang_gs)
  gy_wang = df_w_vol$var
  # gx1 = seq(max(min(gx), -20), max(gx), length.out=100)
  f = splinefun(x = gx_wang, y=gy_wang)
  gs0_wang = df_w_vol$wang_gs[which(df_w_vol$var==0)]
  psi88S_wang = f(log(gs0_wang*0.12))
  psi50S_wang = f(log(gs0_wang*0.50))
  psi12S_wang = f(log(gs0_wang*0.88))
  cat("psi88S_wang = ", psi88S_wang, "\n")
  cat("psi50S_wang = ", psi50S_wang, "\n")
  cat("psi12S_wang = ", psi50S_wang, "\n")

  dpx_wang = df_w_vol$var
  dpy_wang = df_w_vol$wang_dpsi
  f1 = splinefun(dpy_wang~dpx_wang)
  dpx1 = seq(min(dpx), max(dpx), length.out=100)
  dp88S_wang = f1(psi88S_wang)
  dp50S_wang = f1(psi50S_wang)
  dp12S_wang = f1(psi12S_wang)
  cat("psiL88S_wang = ", psi88S_wang-dp88S_wang, "\n")
  cat("psiL50S_wang = ", psi50S_wang-dp50S_wang, "\n")
  cat("psiL12S = ", psi12S_wang-dp12S_wang, "\n")

  cat(psi88S_wang, "\t", psi50S_wang, "\t", psi12S_wang, "\t", psi88S_wang-dp88S_wang, 
      "\t", psi50S_wang-dp50S_wang, psi12S_wang-dp12S_wang, "\n")
  ...
  a_wang_spl = splinefun(x = df_w_vol$var, y=df_w_vol$wang_a)
  g_wang_spl = splinefun(x = df_w_vol$var, y=df_w_vol$wang_gs)
  c_wang_spl = splinefun(x = df_w_vol$var, y=df_w_vol$wang_chi)
  ...
  subdata$a_wang_pred = a_wang_spl(subdata$Predawn.LWP..MPa.)
  subdata$g_wang_pred = g_wang_spl(subdata$Predawn.LWP..MPa.)
  subdata$c_wang_pred = c_wang_spl(subdata$Predawn.LWP..MPa.)

  if (!is.null(dpsi_data)){
    d_spl = splinefun(x = df_w_vol$var, y=df_w_vol$dpsi)
    dpsi_data$d_pred = d_spl(dpsi_data$SWP)
    d_wang_spl = splinefun(x = df_w_vol$var, y=df_w_vol$wang_dpsi)
    dpsi_data$d_wang_pred = d_wang_spl(dpsi_data$SWP)
  ...
  dat$a_wang_pred[ids] <<- subdata$a_wang_pred 
  dat$g_wang_pred[ids] <<- subdata$g_wang_pred 
  dat$c_wang_pred[ids] <<- subdata$c_wang_pred 
    
  p1 <- df_w_vol %>% 
  ggplot() +
  geom_line(aes(x = var, y = vcmax), col="green3", size=1) +
  geom_line(aes(x = var, y = wang_vcmax ), col="grey30", size=1) +
...
p2 <- df_w_vol %>%
  ...
  geom_line(aes(x = var, y = wang_dpsi ), col="grey30", size=1) +
...
p3 <- df_w_vol %>%
  ...
  geom_line(aes(x = var, y = wang_gs ), col="grey30", size=1) +
...
p4 <- df_w_vol %>%
  mutate(chi = ci/out_analytical_ca,
         chi_wang = wang_ci/out_analytical_ca) %>% 
  ggplot() +
  geom_line(aes(x = var, y = chi), col="magenta", size=1)+
  geom_line(aes(x = var, y = chi_wang ), col="grey30", size=1) +
...
p5 <- df_w_vol %>%
  ...
  geom_line(aes(x = var, y = wang_jmax ), col="grey30", size=1) +
  # geom_vline(xintercept = par_plant_std$psi50, col="orange")+
...
p6 <- df_w_vol %>%
  ggplot() +
  geom_line(aes(x = var, y = a), col="green3", size=1) +
  geom_line(aes(x = var, y = wang_a ), col="grey30", size=1) +
...
pmodel_fit = function(species){
  ...
  x_wang = as.numeric(cpar_wang[which(cpar_wang$Species == species), -1])
  # par_plant_wang = list(
  #   # Ks0=1, 
  #   # v_huber=1, 
  #   conductivity= x[1]*1e-16, # in Joshi et al 2021, K is in  10^-16 m 
  ...
  par_plant_wang = list(
    # Ks0=1,
    # v_huber=1,
    conductivity= x_wang[1]*1e-16, # in Joshi et al 2021, K is in  10^-16 m
    # height=1,
    psi50 = x_wang[2],
    # b=x_wang[3]
    b=x[3]
  ...
  # )
  # par_cost_now = list(
  #   alpha  = 0.08,          # cost of Jmax
  #   gamma = 1    # cost of hydraulic repair.  
  # )
  
  # par_plant_now = par_plant_wang
  # par_cost_now = par_cost_wang
  ...
  if (cpar[which(cpar$Species == species), "inst"] == 0){
    ...
    dat1_wang = tibble(var = seq(-6,0, length.out = 30)) %>% 
      mutate(pmod = purrr::map(var, 
                          ~ model_numerical(tc = mean(data$T..deg.C.),
                                            ...
                                            psi_soil = ., 
                                            rdark = 0.02, 
                                            par_plant = par_plant_wang, 
                                            par_cost = par_cost_wang,
                                            stomatal_model = stomatal_model))) %>% 
      unnest_wider(pmod) %>% 
      rename_with(~paste0("wang_",.), c(2:10))
    
    
      dat1 <- dat1 %>% left_join(dat1_wang, by = "var")
    ...
    dat_acc = tibble(var = spl(day)) %>% 
      ...
      dat_acc_wang = tibble(var = spl(day)) %>% 
      mutate(pmod = map(var,
                          ~ model_numerical(tc = mean(data$T..deg.C.),
                                            ppfd = mean(data$Iabs.growth.in.growth.chamber..umol.m.2.s.1.), 
                                            vpd = mean(data$D..unitless...Pa...Pa.atm..*101325), 
    ...
    dat1_wang = tibble(var = lwp, jmax_a=dat_acc_wang$jmax, vcmax_a=dat_acc_wang$vcmax) %>%
      mutate(p = purrr::pmap(list(var, jmax_a, vcmax_a), 
                             ~model_numerical_instantaneous(
                                                    ...
                                                    stomatal_model = stomatal_model)) ) %>% 
      unnest_wider(p) %>% 
      rename_with(~paste0("wang_",.), c(2:13))
    
      dat1 <- dat1 %>% left_join(dat1_wang, by = "var")
      
    par(mfrow=c(3,2), mar=c(4,4,1,1))
    matplot(x=c(max(day):0, -1:-(k-1)), 
            y=cbind(c(lwp_week, rep(NA,6)), lwp_day(c(max(day):0, rep(0,k-1)))), 
...
p5 =
  dat %>% ggplot(mapping = aes(x=a_wang_pred, y=A..umol.m.2.s.1.)) + 
  mytheme()+
  geom_point(aes(fill=-Predawn.LWP..MPa./pgL88), shape=21, color=makeTransparent("black",0)) + 
  geom_smooth(method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  ...
  scale_fill_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  labs(fill = expression(psi[s]*"/|"*psi[g88]*"|"))+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dat$A..umol.m.2.s.1., dat$a_wang_pred, use = "pairwise.complete.obs")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
...
p6 = dat %>% ggplot(mapping = aes(x=g_wang_pred, y=gC..mol.m.2.s.1.)) +   
  mytheme()+
  geom_point(aes(fill=-Predawn.LWP..MPa./pgL88), shape=21, color=makeTransparent("black",0)) + 
  geom_smooth(method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  ...
  scale_fill_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  labs(fill = expression(psi[s]*"/|"*psi[g88]*"|"))+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dat$gC..mol.m.2.s.1., dat$g_wang_pred, use = "p")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
...
p7 = dat %>% ggplot(mapping = aes(x=c_wang_pred, y=Ciest/ca..ppm.)) +   
  mytheme()+
  geom_point(aes(fill=-Predawn.LWP..MPa./pgL88), shape=21, color=makeTransparent("black",0)) + 
  geom_smooth(data = dat[which(dat$Predawn.LWP..MPa./dat$pgL88 < 1),], method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  # scale_fill_gradient(oob=squish, limits = c(-4,0), low = makeTransparent("goldenrod1"), high = makeTransparent("blue"))+
  scale_fill_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dat$c_wang_pred[-dat$Predawn.LWP..MPa./dat$pgL88>-1], (dat$Ciest/dat$ca..ppm.)[-dat$Predawn.LWP..MPa./dat$pgL88>-1], use = "pairwise.complete.obs")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
  labs(fill = expression(psi[s]*"/|"*psi[g88]*"|"))+
...
p8 = dpsi_df %>% ggplot() +   
  mytheme()+
  geom_point(mapping = aes(x=d_wang_pred, y=Dpsi, color=-SWP/pgL88, shape=source, size = source)) + 
  geom_smooth(mapping=aes(x=d_wang_pred, y=Dpsi), method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  # scale_color_gradient(low = makeTransparent("goldenrod1"), high = makeTransparent("blue"))+
  scale_color_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  scale_size_manual(values=c(1.5,1))+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dpsi_df$Dpsi, dpsi_df$d_wang_pred, use = "pairwise.complete.obs")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
...
cowplot::plot_grid(p1,p5, p2,p6, p3,p7, p4,p8, labels=LETTERS, label_size = 16, label_colour = "grey50", label_x = 0.05, align = "hv", rel_widths = 1, ncol=2)
# dev.off()
