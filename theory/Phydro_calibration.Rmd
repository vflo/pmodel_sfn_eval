...
author: "Jaideep Joshi and Victor Flo"
date: "`r Sys.Date()`"
output:
  html_document: null
  toc: yes
...
dat$g_pred = NA
dat$c_pred = NA
dat$a_scheme_pred = NA
dat$g_scheme_pred = NA
dat$c_scheme_pred = NA
dpsi_df = read.csv("../DATA/drying_experiments_dpsi_Joshi_et_al_2022.csv")
dpsi_df$d_scheme_pred <- NA
...
cpar_scheme = read.csv("../DATA/cpar_parameters.csv")
...
model_scheme <- "phydro_sperry"
```

```{r echo=F}
plot_all = function(df_w_vol, varname, species, b,a, p_crit){
  ...
  #scheme
  gx_scheme = log(df_w_vol$scheme_gs)
  gy_scheme = df_w_vol$var
  # gx1 = seq(max(min(gx), -20), max(gx), length.out=100)
  f = splinefun(x = gx_scheme, y=gy_scheme)
  gs0_scheme = df_w_vol$scheme_gs[which(df_w_vol$var==0)]
  psi88S_scheme = f(log(gs0_scheme*0.12))
  psi50S_scheme = f(log(gs0_scheme*0.50))
  psi12S_scheme = f(log(gs0_scheme*0.88))
  cat("psi88S_scheme = ", psi88S_scheme, "\n")
  cat("psi50S_scheme = ", psi50S_scheme, "\n")
  cat("psi12S_scheme = ", psi50S_scheme, "\n")
  
  dpx_scheme = df_w_vol$var
  dpy_scheme = df_w_vol$scheme_dpsi
  f1 = splinefun(dpy_scheme~dpx_scheme)
  dpx1 = seq(min(dpx), max(dpx), length.out=100)
  dp88S_scheme = f1(psi88S_scheme)
  dp50S_scheme = f1(psi50S_scheme)
  dp12S_scheme = f1(psi12S_scheme)
  cat("psiL88S_scheme = ", psi88S_scheme-dp88S_scheme, "\n")
  cat("psiL50S_scheme = ", psi50S_scheme-dp50S_scheme, "\n")
  cat("psiL12S = ", psi12S_scheme-dp12S_scheme, "\n")
  
  cat(psi88S_scheme, "\t", psi50S_scheme, "\t", psi12S_scheme, "\t", psi88S_scheme-dp88S_scheme, 
      "\t", psi50S_scheme-dp50S_scheme, psi12S_scheme-dp12S_scheme, "\n")
  
  
  subdata = dat %>% filter(Species == species) %>% filter(Predawn.LWP..MPa. > -5)
  dpsi_data = dpsi_df %>% filter(Species == species) %>% filter(SWP > -5)
  ...
  a_scheme_spl = splinefun(x = df_w_vol$var, y=df_w_vol$scheme_a)
  g_scheme_spl = splinefun(x = df_w_vol$var, y=df_w_vol$scheme_gs)
  c_scheme_spl = splinefun(x = df_w_vol$var, y=df_w_vol$scheme_chi)
  ...
  subdata$a_scheme_pred = a_scheme_spl(subdata$Predawn.LWP..MPa.)
  subdata$g_scheme_pred = g_scheme_spl(subdata$Predawn.LWP..MPa.)
  subdata$c_scheme_pred = c_scheme_spl(subdata$Predawn.LWP..MPa.)
  
  if (!is.null(dpsi_data)){
    d_spl = splinefun(x = df_w_vol$var, y=df_w_vol$dpsi)
    dpsi_data$d_pred = d_spl(dpsi_data$SWP)
    d_scheme_spl = splinefun(x = df_w_vol$var, y=df_w_vol$scheme_dpsi)
    dpsi_data$d_scheme_pred = d_scheme_spl(dpsi_data$SWP)
  ...
  dat$a_scheme_pred[ids] <<- subdata$a_scheme_pred 
  dat$g_scheme_pred[ids] <<- subdata$g_scheme_pred 
  dat$c_scheme_pred[ids] <<- subdata$c_scheme_pred 
  
  df_w_vol <- df_w_vol %>%
    mutate(chi = ci/out_analytical_ca,
           chi_scheme = scheme_ci/out_analytical_ca)
  
  p1 <- ggplot() +
    geom_line(data = df_w_vol, aes(x = var, y = vcmax), col="green3", size=1) +
    geom_line(data = df_w_vol %>% filter(var > p_crit), aes(x = var, y = scheme_vcmax ), col="grey30", size=1) +
    geom_line(data = df_w_vol,aes(x = var, y = out_analytical_vcmax ), col="grey", size=1) +
  ...
  p2 <- df_w_vol %>%
    ...
    geom_line(data = df_w_vol %>% filter(var > p_crit), aes(x = var, y = scheme_dpsi ), col="grey30", size=1) +
  ...
  p3 <- ggplot() +
    geom_line(data = df_w_vol,aes(x = var, y = gs), col="cyan2", size=1)+
    geom_line(data = df_w_vol %>% filter(var > p_crit), aes(x = var, y = scheme_gs ), col="grey30", size=1) +
    geom_line(data = df_w_vol, aes(x = var, y = out_analytical_gs), col="grey", size=1)+
    geom_point(data = tibble(x=seq(-5,0,length.out=100), y=g_spl(seq(-5,0,length.out=100)) ), aes(x=x,y=y), size=0.5, col="cyan3")+
  ...
  p4 <- ggplot() +
    geom_line(data = df_w_vol, aes(x = var, y = chi), col="magenta", size=1)+
    geom_line(data = df_w_vol %>% filter(var > p_crit), aes(x = var, y = chi_scheme ), col="grey30", size=1) +
    geom_line(data = df_w_vol, aes(x = var, y = out_analytical_chi), col="grey", size=1)+
    geom_point(data = tibble(x=seq(-5,0,length.out=100), y=c_spl(seq(-5,0,length.out=100)) ), aes(x=x,y=y), size=0.5, col="magenta3")+
  ...
  p5 <- df_w_vol %>%
    ggplot() +
    geom_line(data = df_w_vol, aes(x = var, y = jmax), col="goldenrod1", size=1) +
    geom_line(data = df_w_vol %>% filter(var > p_crit), aes(x = var, y = scheme_jmax ), col="grey30", size=1) +
    # geom_vline(xintercept = par_plant_std$psi50, col="orange")+
  ...
  p6 <- df_w_vol %>%
    ggplot() +
    geom_line(data = df_w_vol, aes(x = var, y = a), col="green3", size=1) +
    geom_line(data = df_w_vol %>% filter(var > p_crit), aes(x = var, y = scheme_a ), col="grey30", size=1) +
    geom_line(data = df_w_vol, aes(x = var, y = out_analytical_gpp), col="grey",size=1) +
    geom_point(data = tibble(x=seq(-5,0,length.out=100), y=a_spl(seq(-5,0,length.out=100)) ), aes(x=x,y=y), size=0.5, col="darkgreen")+
...
pmodel_calibrate_numerical <- function(tc, ppfd, vpd, co2, elv, fapar, kphio,
                                       psi_soil, rdark, par_plant, 
                                       par_cost = NULL, opt_hypothesis = "PM"){
  
  if (is.null(par_cost)) par_cost = list(alpha=0.1, gamma=1)

  out_hydraulics = rphydro_numerical(tc, ppfd, vpd, co2, elv, fapar, kphio, 
                                     psi_soil, rdark, par_plant, par_cost)
...
pmodel_fit = function(species, accl, s_model){
  ...
  x_scheme = cpar_scheme%>% 
    filter(Species == species,
           Flo_2022 == 1, 
           acclimation == accl,
           stomatal_model == s_model)
  x_scheme <- as.numeric(x_scheme[c(2:6)])

  par_plant_scheme = list(
    conductivity= x_scheme[1]*1e-16, # in Joshi et al 2021, K is in  10^-16 m
    psi50 = x_scheme[2],
    b=x_scheme[3]
  )
  par_cost_scheme = list(
    alpha  = x_scheme[4]         # cost of Jmax
  )
  p_crit_scheme = par_plant_scheme$psi50 * (log(10000)/log(2)) ^ ( 1/par_plant_scheme$b)
  
  if (cpar[which(cpar$Species == species), "inst"] == 0){
    cat("Acclimated response: days = ", data$Drydown.days, "\n")
    dat1 = tibble(var = seq(-5,0, length.out = 30)) %>% 
      mutate(p = purrr::map(var, 
                            ~pmodel_calibrate_analytical(tc = mean(data$T..deg.C.),
                                                         ppfd = mean(data$Iabs.growth.in.growth.chamber..umol.m.2.s.1.), 
                                                         vpd = mean(data$D..unitless...Pa...Pa.atm..*101325), 
                                                         co2 = mean(data$ca..ppm.), 
                                                         elv = 0, 
                                                         fapar = .99, 
                                                         kphio = 0.087,
                                                         psi_soil = ., 
      ...
      unnest_wider(p) %>% 
      unnest_wider(out_hydraulics) %>%
      unnest_wider(out_analytical, names_sep = "_")
    
    dat1_scheme = tibble(var = seq(-5,0, length.out = 30)) %>% 
      mutate(pmod = purrr::map(var, 
                               ~ model_numerical(tc = mean(data$T..deg.C.),
                                                 ppfd = mean(data$Iabs.growth.in.growth.chamber..umol.m.2.s.1.), 
                                                 vpd = mean(data$D..unitless...Pa...Pa.atm..*101325), 
                                                 co2 = mean(data$ca..ppm.), 
                                                 elv = 0, 
                                                 ...
                                                 psi_soil = ., 
                                                 rdark = 0.02, 
                                                 par_plant = par_plant_scheme, 
                                                 par_cost = par_cost_scheme,
                                                 stomatal_model = s_model))) %>% 
      unnest_wider(pmod) %>% 
      rename_with(~paste0("scheme_",.), c(2:10))
    
    
    dat1 <- dat1 %>% left_join(dat1_scheme, by = "var")
    ...
    # Calculate week-average LWP corresponding to days from 0 to last day (corresponding to -5 MPa). Days before start (-ve days) are considered well-watered
    k = 7
    ...
    dat_acc = tibble(var = spl(day)) %>% 
      mutate(pmod = map(var,
                        # ~ model_numerical(tc = mean(data$T..deg.C.),
                        ~rphydro_analytical(tc = mean(data$T..deg.C.),
                                            ppfd = mean(data$Iabs.growth.in.growth.chamber..umol.m.2.s.1.), 
                                            vpd = mean(data$D..unitless...Pa...Pa.atm..*101325), 
                                            co2 = mean(data$ca..ppm.), 
                                            elv = 0, 
    ...
    dat_acc_scheme = tibble(var = spl(day)) %>% 
      mutate(pmod = map(var,
                        ~ model_numerical(tc = mean(data$T..deg.C.),
                                          ppfd = mean(data$Iabs.growth.in.growth.chamber..umol.m.2.s.1.), 
                                          vpd = mean(data$D..unitless...Pa...Pa.atm..*101325), 
    ...
    dat1 = tibble(var = lwp, jmax_a=dat_acc$jmax, vcmax_a=dat_acc$vcmax) %>%
      mutate(p = purrr::pmap(list(var, jmax_a, vcmax_a), 
                             ~pmodel_calibrate_inst(jmax = ..2,
                                                    vcmax = ..3, 
                                                    tc = mean(data$T..deg.C.), 
                                                    ppfd = mean(data$Iabs.in.measurement.chamber..liCor...umol.m.2.s.1.), 
                                                    vpd = mean(data$D..unitless...Pa...Pa.atm..*101325),
                                                    co2 = mean(data$ca..ppm.), 
                                                    elv = 0, 
                                                    fapar = .99, 
    ...
    dat1_scheme = tibble(var = lwp, jmax_a=dat_acc_scheme$jmax, vcmax_a=dat_acc_scheme$vcmax) %>%
      mutate(p = purrr::pmap(list(var, jmax_a, vcmax_a), 
                             ~model_numerical_instantaneous(
                               ...
                               stomatal_model = s_model)) ) %>% 
      unnest_wider(p) %>% 
      rename_with(~paste0("scheme_",.), c(2:13))
    
    dat1 <- dat1 %>% left_join(dat1_scheme, by = "var")
    
    par(mfrow=c(3,2), mar=c(4,4,1,1))
    matplot(x=c(max(day):0, -1:-(k-1)), 
            y=cbind(c(lwp_week, rep(NA,5)), lwp_day(c(max(day):0, rep(0,k-1)))), 
            lty=1, lwd=2:1, col=c("red3", "black"), type="l", xlab="day", ylab="SWP")
    points(lwp~day)
  ...
  dat1 %>% plot_all(varname = "psi_soil", species=species, b, a, p_crit_scheme)
  
  # dat2 = tibble(var = seq(-5,0, length.out = 30)) %>% 
...
pmodel_fit(species, 1, model_scheme)
```
...
write.csv(x=dat, file="../DATA/pred_obs_a_g_c.csv")
write.csv(x=dpsi_df, file = "../DATA/dpsi_df.csv")

dat = read.csv(file="../DATA/pred_obs_a_g_c.csv")
dpsi_df = read.csv(file = "../DATA/dpsi_df.csv")

dat = left_join(x = dat, y= cpar, by="Species")
dpsi_df = left_join(x = dpsi_df, y= cpar, by="Species")

dat = left_join(x = dat, y= cpar_scheme %>% 
                  filter(Flo_2022 == 1, acclimation == 1, stomatal_model == model_scheme), 
                by="Species")
dpsi_df = left_join(x = dpsi_df, y= cpar_scheme%>% 
                  filter(Flo_2022 == 1, acclimation == 1, stomatal_model == model_scheme), 
                  by="Species")
...
p5 =
  dat %>% ggplot(mapping = aes(x=a_scheme_pred, y=A..umol.m.2.s.1.)) + 
  mytheme()+
  geom_point(aes(fill=-Predawn.LWP..MPa./pgL88), shape=21, color=makeTransparent("black",0)) + 
  geom_smooth(method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  ...
  scale_fill_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  labs(fill = expression(psi[s]*"/|"*psi[g88]*"|"))+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dat$A..umol.m.2.s.1., dat$a_scheme_pred, use = "pairwise.complete.obs")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
  xlab(expression(textstyle("Predicted A Wang")))+
  ylab(expression(textstyle("Observed A")))+
  ggtitle(expression(atop("Assimilation rate,", italic(A)*" ("*mu*"mol m"^-2~"s"^-1*")")))
...
p6 = dat %>% ggplot(mapping = aes(x=g_scheme_pred, y=gC..mol.m.2.s.1.)) +   
  mytheme()+
  geom_point(aes(fill=-Predawn.LWP..MPa./pgL88), shape=21, color=makeTransparent("black",0)) + 
  geom_smooth(method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  ...
  scale_fill_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  labs(fill = expression(psi[s]*"/|"*psi[g88]*"|"))+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dat$gC..mol.m.2.s.1., dat$g_scheme_pred, use = "p")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
  xlab(expression("Predicted "*g[s]*" Wang"))+
  ylab(expression("Observed "*g[s]))+
  ggtitle(expression(atop("Stomatal conductance,","g"[s]~"(mol m"^-2~"s"^-1*")")))
  # ylim(0, 0.3)
...
p7 = dat %>% ggplot(mapping = aes(x=c_scheme_pred, y=Ciest/ca..ppm.)) +   
  mytheme()+
  geom_point(aes(fill=-Predawn.LWP..MPa./pgL88), shape=21, color=makeTransparent("black",0)) + 
  geom_smooth(data = dat[which(dat$Predawn.LWP..MPa./dat$pgL88 < 1),], method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  # scale_fill_gradient(oob=squish, limits = c(-4,0), low = makeTransparent("goldenrod1"), high = makeTransparent("blue"))+
  scale_fill_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dat$c_scheme_pred[-dat$Predawn.LWP..MPa./dat$pgL88>-1], (dat$Ciest/dat$ca..ppm.)[-dat$Predawn.LWP..MPa./dat$pgL88>-1], use = "pairwise.complete.obs")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
  labs(fill = expression(psi[s]*"/|"*psi[g88]*"|"))+
  xlab(expression("Predicted"*chi*"Wang"))+
  ylab(expression("Observed"*chi))+
  ggtitle(expression(atop("Leaf internal-to-","ambient CO"[2]~"ratio,"~italic(chi))))
...
p8 = dpsi_df %>% ggplot() +   
  mytheme()+
  geom_point(mapping = aes(x=d_scheme_pred, y=Dpsi, color=-SWP/pgL88, shape=source, size = source)) + 
  geom_smooth(mapping=aes(x=d_scheme_pred, y=Dpsi), method = "lm", se = F, col="black")+
  geom_abline(slope = 1, intercept=0, color="grey") +
  # scale_color_gradient(low = makeTransparent("goldenrod1"), high = makeTransparent("blue"))+
  scale_color_viridis_c(oob=squish, limits = c(-1,0), direction = -1, begin=.25)+
  scale_size_manual(values=c(1.5,1))+
  annotate(size=4, geom="text", label = paste("italic(R)^2 == ", sprintf("%.2f", cor(dpsi_df$Dpsi, dpsi_df$d_scheme_pred, use = "pairwise.complete.obs")^2)), x=Inf, y=-Inf, hjust=1.2, vjust=-1.2, parse=T)+
  xlab(expression("Predicted"~Delta*psi~"Wang"))+
  ylab(expression("Observed"~Delta*psi)) + 
  ggtitle(expression(atop("Soil-leaf water-potential","difference,"~italic(Delta*psi)~"(MPa)")))+
  labs(color=expression(psi[s]*"/|"*psi[g88]*"|"), size="Source", shape="Source")+
  ylim(c(0,NA))
