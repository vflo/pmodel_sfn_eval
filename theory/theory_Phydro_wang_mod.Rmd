...
$$
F = A (\frac{E_{crit} - E_{actual}}{E_{crit}})- \alpha J_\mathrm{max}
$$
...
# PHYDRO WANG
#################################################################################################
fn_profit_phydro_wang_psi_soil <- function(par, par_cost, par_photosynth, par_plant, par_env, do_optim = FALSE, opt_hypothesis){
  ...
  out <- tibble(out_ecrit = a,
                out_e = a*e_e_crit+cost,
...
calc_phydro_wang_test <- function(tc, ppfd, vpd, co2, elv, LAI, fapar, kphio, psi_soil, rdark, par_plant,  par_cost = NULL){
  ...
  lj_dps = optimise_stomata_phydro_test(fn_profit_phydro_wang_test, 
    ...
    gs = calc_gs_test(dpsi, psi_soil, par_plant, par_env) * par_env$LAI 
...
df_res %>% 
  ggplot()+
  geom_point(aes(dpsi,out_e), color = "grey50")+
  geom_point(aes(dpsi,out_ecrit), color = "grey50")+
...
$$
F = A_j - A_j\frac{E}{E_\mathrm{crit}}- \alpha J_\mathrm{max}
$$


and then
$$
F = g_\mathrm{s}\,  c_\mathrm{a}\, (1-\chi)  - g_\mathrm{s} \, c_\mathrm{a}\, (1-\chi)\,\frac{E}{E_\mathrm{crit}}- \alpha J_\mathrm{max}
$$

$E$ can be calculated as the stand conductance ($K$) multiplied by the gradient between the soil and the leaf water potential.
$$
E = K_{max}  \int_{\psi_\mathrm{s}}^{\psi_\mathrm{s}-\Delta\psi} P(\psi) \,d\psi
$$

Critical transpiration ($E_{crit}$) is equivalent to the maximum transpiration for a given soil potential.
$$
E_\mathrm{crit} = K_{max}  \int_{\psi_\mathrm{s}}^{\infty} P(\psi) \,d\psi
$$
...
$$
\frac{E}{E_\mathrm{crit}} = \frac{ \int_{\psi_\mathrm{s}}^{\psi_\mathrm{s}-\Delta\psi} P(\psi) \,d\psi}{ \int_{\psi_\mathrm{s}}^{\infty} P(\psi) \,d\psi} 
$$
...
$$
F = g_\mathrm{s}\,  c_\mathrm{a} \,(1-\chi)  - g_\mathrm{s} \, c_\mathrm{a} \,(1-\chi) \, \frac{ \int_{\psi_\mathrm{s}}^{\psi_\mathrm{s}-\Delta\psi} P(\psi) \,d\psi}{ \int_{\psi_\mathrm{s}}^{\infty} P(\psi) \,d\psi}  - \alpha J_\mathrm{max} 
$$

Solved by $\chi$
$$
\frac{\partial F}{\partial \chi} = - \alpha \frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \chi} - \frac{g_s c_a\,\int_{\psi_\mathrm{s}}^{\psi_\mathrm{s}-\Delta\psi} P(\psi) \,d\psi}{\int_{\psi_\mathrm{s}}^{\infty} P(\psi) \,d\psi} = 0
$$

Solved by $\Delta\psi$
$$
\frac{\partial F}{\partial \Delta\psi} = \frac{c_a (1-\chi)\,(\frac{\partial g_s}{\partial \Delta\psi}\,(\int_{\psi_\mathrm{s}}^{\psi_\mathrm{s}-\Delta\psi} P(\psi) \,d\psi+g_s\,\frac{1}{2} ^{(\frac{\psi_\mathrm{s}-\Delta\psi}{\psi_{50}})^b}))}{\int_{\psi_\mathrm{s}}^{\infty} P(\psi) \,d\psi} - \alpha \frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \Delta\psi}= 0
$$
...
ggplot()+
  ...
  geom_line(data = df_res1, aes(dpsi,out_e, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res1,aes( dpsi,out_ecrit, color = jmax, group = jmax), linetype =4)+
...
ggplot()+
  ...
  # geom_point(aes(dpsi,out_e), color = "#112CB9")+
  # geom_point(aes(dpsi,out), color = "#FFB806")+
  geom_line(data = df_res1, aes(gs,out_e, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res1,aes(gs,out_ecrit, color = jmax, group = jmax), linetype =4)+
...
ggplot()+
  ...
  # geom_point(aes(dpsi,out_e), color = "#112CB9")+
  # geom_point(aes(dpsi,out), color = "#FFB806")+
  geom_line(data = df_res2, aes(-psi_soil+dpsi,out_e, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res2,aes( -psi_soil+dpsi,out_ecrit, color = jmax, group = jmax), linetype =4)+
...
ggplot()+
  ...
  # geom_point(aes(dpsi,out_e), color = "#112CB9")+
  # geom_point(aes(dpsi,out), color = "#FFB806")+
  geom_line(data = df_res2, aes(gs,out_e, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res2,aes(gs,out_ecrit, color = jmax, group = jmax), linetype =4)+
...
par %>% 
  split(seq(nrow(.))) %>% 
  purrr::map_df(function(x){
    fn_profit_phydro_wang_psi_soil(x,
                 par_cost = par_cost, 
                 par_photosynth = par_photosynth, 
                 par_plant = par_plant, 
                 par_env = par_env, 
...
dat_soilm_numerical = tibble(var=seq(-6,0,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_wang_test(tc = 25, ppfd = 400, vpd = 1000, co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = .,  par_plant = list(conductivity = 1e-16,psi50 = -1.46, b=1), rdark = 0.02, par_cost = list(alpha=0.1))),
         out_analytical = purrr::map(var, ~calc_phydro_test(tc = 25, ppfd = 400, vpd = 1000, co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = .,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha = 0.1, gamma = 1)))) %>%
...
dat_vpd_numerical = tibble(var=seq(100,3000,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_wang_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -1,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1))),
         out_analytical = purrr::map(var, ~calc_phydro_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -1,  par_plant = list(conductivity = 1e-16,psi50 = -1.46, b=1), rdark = 0.02, par_cost = list(alpha = 0.1, gamma = 1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")%>%
  unnest_wider(out_analytical, names_sep = "_")
...
dat_vpd2_numerical = tibble(var=seq(100,3000,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_wang_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -3,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1))),
         out_analytical = purrr::map(var, ~calc_phydro_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -3,  par_plant = list(conductivity = 1e-16,psi50 = -1.46, b=1), rdark = 0.02, par_cost = list(alpha = 0.1, gamma = 1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")%>%
  unnest_wider(out_analytical, names_sep = "_")
