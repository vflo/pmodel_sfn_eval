...
The SOX stomatal scheme (Eller et al. 2018) is obtimised as: (old analytical solution NOT SOLVED YET)
$$
F = A_j  (\frac{k_\mathrm{\psi_l}-k_\mathrm{\psi_{crit}}}{k_\mathrm{\psi_s}-k_\mathrm{\psi_{crit}}})
$$

Then, accounting for photosynthetic cost:
$$
F = (A_j- \alpha J_\mathrm{max})  (\frac{k_\mathrm{\psi_l}-k_\mathrm{\psi_{crit}}}{k_\mathrm{\psi_s}-k_\mathrm{\psi_{crit}}})
$$
...
fn_profit_phydro_sox_test <- function(par, psi_soil, e_crit, p_crit, par_cost, 
  ...
  vcmax = calc_vcmax_coordinated_numerical(a,ci, par_photosynth)
    ...
    reduction_factor = (kl - k_crit)/(K - k_crit)
    cost = (par_cost$alpha * jmax)
    out <- exp((a- cost)*reduction_factor) 
...
df_res %>% 
  ggplot()+
  geom_point(aes(dpsi,a*risk), color = "grey50")+
  geom_point(aes(dpsi,(cost*risk)), color = "grey50")+
...
$$
F = g_\mathrm{s}  c_\mathrm{a} (1-\chi) (\frac{(\frac{1}{2})^{(\frac{\psi_s-\Delta\psi}{\psi_{50}})^b}}{(\frac{1}{2})^{(\frac{\psi_s}{\psi_{50}})^b}}) - \alpha J_\mathrm{max}(\frac{(\frac{1}{2})^{(\frac{\psi_s-\Delta\psi}{\psi_{50}})^b}}{(\frac{1}{2})^{(\frac{\psi_s}{\psi_{50}})^b}})
$$


Solved by $\chi$
$$
\frac{\partial F}{\partial \chi} = (\frac{1}{2})^{(\frac{\psi_s-\Delta\psi}{\psi_{50}})^b-(\frac{\psi_s}{\psi_{50}})^b}(-\alpha\frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \chi} - c_a\,g_s) = 0
$$

Solved by $\Delta\psi$
$$
\frac{\partial F}{\partial \Delta\psi} = \frac{1}{\psi_{50}}(\frac{1}{2})^{(\frac{\psi_s-\Delta\psi}{\psi_{50}})^b-(\frac{\psi_s}{\psi_{50}})^b}\,(\alpha\,(ln(2)\,b\,J_{max}(\frac{\psi_s-\Delta\psi}{\psi_{50}})^{b-1}-\psi_{50}\frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \Delta\psi})+c_a(g_s\,(ln(2)\,\chi-ln(2))\,b\,(\frac{\psi_s-\Delta\psi}{\psi_{50}})^{b-1}-\psi_{50}(\chi-1)\frac{\partial g_s}{\partial \Delta\psi}))=0
