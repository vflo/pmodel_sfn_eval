...
$$
k_\mathrm{\psi_l} = K_{max}\;(\frac{1}{2})^{(\frac{\psi_l}{\psi_{50}})^b} = K_{max}\;(\frac{1}{2})^{(\frac{ \psi_s -\Delta\psi}{\psi_{50}})^b}
$$


We assume that $k_\mathrm{\psi_{crit}}$ is equal to 0.
...
$$
F = g_\mathrm{s}  c_\mathrm{a} (1-\chi) - A_{max} \frac{((\frac{1}{2})^{(\frac{\psi_s}{\psi_{50}})^b}- (\frac{1}{2})^{(\frac{ \psi_s -\Delta\psi}{\psi_{50}})^b})}{(\frac{1}{2})^{(\frac{\psi_s}{\psi_{50}})^b}} -  A_{max} \alpha J_\mathrm{max}
$$


Solved by $\chi$
$$
\frac{\partial F}{\partial \chi} = A_\mathrm{max}(\frac{ln(2)\,(\frac{1}{2})^{(\frac{ \psi_s -\Delta\psi}{\psi_{50}})^b -{(\frac{\psi_s}{\psi_{50}})^b}}\,b\,(\frac{ \psi_s -\Delta\psi}{\psi_{50}})^b}{\psi_s -\Delta\psi} - \alpha \frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \chi})+\frac{\partial A_\mathrm{max}}{\partial \chi}\,(-\alpha\,J_\mathrm{max} +  (\frac{1}{2})^{(\frac{ \psi_s -\Delta\psi}{\psi_{50}})^b -{(\frac{\psi_s}{\psi_{50}})^b}}-1)-g_s\,c_a= 0
$$

Solved by $\Delta\psi$
$$
\frac{\partial F}{\partial \Delta\psi} =  \frac{\partial A_\mathrm{max}}{\partial \Delta\psi} \,(-\alpha\,J_\mathrm{max} +  (\frac{1}{2})^{(\frac{ \psi_s -\Delta\psi}{\psi_{50}})^b -{(\frac{\psi_s}{\psi_{50}})^b}}-1) - A_\mathrm{max}\,\alpha\,\frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \Delta\psi}+\frac{\partial g_s}{\partial \Delta\psi} c_a (1-\chi) = 0
$$
...
par %>% 
  split(seq(nrow(.))) %>% 
  purrr::map_df(function(x){
    fn_profit_phydro_sperry_psi_soil(x,
                 par_cost = par_cost, 
                 par_photosynth = par_photosynth, 
                 par_plant = par_plant, 
                 par_env = par_env, 
...
dat_soilm_numerical = tibble(var=seq(-6,0,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_sperry_test(tc = 25, ppfd = 400, vpd = 1000, co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = .,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")
...
dat_vpd_numerical = tibble(var=seq(100,3000,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_sperry_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -1,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")

dat_vpd2_numerical = tibble(var=seq(100,3000,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_sperry_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -3,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")
