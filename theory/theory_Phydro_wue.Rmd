...
$$
F = A_j - \gamma \,E - \alpha J_\mathrm{max}
$$
...
# PHYDRO WUE
#################################################################################################
fn_profit_phydro_wue_psi_soil <- function(par, par_cost, par_photosynth, par_plant, par_env, do_optim = FALSE, opt_hypothesis){
  ...
  risk = par_cost$gamma*e*1e3
  out <- tibble(risk = risk,
                cost = cost,
                out= a-risk-cost,
                vcmax = vcmax,
                jmax = jmax,
...
fn_profit_phydro_wue <- function(par, psi_soil, par_cost, par_photosynth, par_plant, par_env, do_optim = FALSE, opt_hypothesis){
  
  jmax = exp(par[1])$logjmax  # Jmax in umol/m2/s (logjmax is supplied by the optimizer)
  dpsi = par[2]$dpsi       # delta Psi in MPa
  K = scale_conductivity_ks(par_plant$conductivity, par_env, par_plant, do_backtransform = FALSE)
  gs = calc_gs_test(dpsi, psi_soil, par_plant, par_env) * par_env$LAI  # gs in mol/m2ground/s
  e  = 1.6*gs*(par_env$vpd/par_env$patm)*1e3         # E in mol/m2ground/s
  ...
  out = a-(par_cost$gamma*e) - par_cost$alpha*jmax

  out <- tibble(out_a = a,
                out_cost = (par_cost$gamma*e)+par_cost$alpha*jmax,
                out = a-(par_cost$gamma*e) - par_cost$alpha*jmax, #umolco2 m-2 s-1
                vcmax = vcmax,
                jmax = jmax,
                a = a,
                E = e,
...
fn_profit_phydro_wue_test <- function(par, psi_soil, par_cost, p_crit, e_crit,
  ...
  # print(paste('jmax:', jmax ,'dpsi:',dpsi))
  # if(jmax<=0 |jmax>7 |dpsi<=0 |dpsi>20){out = -1e3}else{
  gs = calc_gs_test(dpsi, psi_soil, par_plant, par_env) * par_env$LAI  # gs in mol/m2ground/s
  e  = 1.6*gs*(par_env$vpd/par_env$patm)         # E in mol/m2ground/s
...
calc_phydro_wue_test <- function(tc, ppfd, vpd, co2, elv, LAI, fapar, kphio, psi_soil, rdark, par_plant,  par_cost = NULL){
  ...
  jmax_lim = exp(7)
    # 3. Optimizer
  lj_dps = optimise_stomata_phydro_test(fn_profit_phydro_wue_test, 
                                     psi_soil = psi_soil, 
                                     e_crit = e_crit,
                                     p_crit= p_crit,
                                     par_cost  = par_cost, 
...
df_res %>% 
  ggplot()+
  geom_point(aes(dpsi,out_cost), color = "grey50")+
  geom_point(aes(dpsi,out_a), color = "grey50")+
...
The components of the water use efficiency maximization function can be rewritten as:
$$
...
and then
$$
F = g_\mathrm{s}  c_\mathrm{a} (1-\chi) -\gamma\,K_{max}  \int_{\psi_\mathrm{s}}^{\psi_\mathrm{s}-\Delta\psi} P(\psi) \,d\psi- \alpha J_\mathrm{max} 
$$

Solved by $\chi$
$$
\frac{\partial F}{\partial \chi} =  -\alpha \frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \chi} - g_a\,c_a= 0
$$

Solved by $\Delta\psi$
$$
\frac{\partial F}{\partial \Delta\psi} = -\alpha \frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \Delta\psi}+\frac{\partial g_s}{\partial \Delta\psi}\,c_a(1-\chi)-\gamma\,K_{max}\,\frac{1}{2} ^{(\frac{\psi_s - \Delta\psi}{\psi_{50}})^b}= 0
$$
...
ggplot()+
  geom_line(data = df_res1, aes(dpsi,out_a, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res1,aes( dpsi,out_cost, color = jmax, group = jmax), linetype =4)+
...
ggplot()+
  geom_line(data = df_res1, aes(gs,out_a, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res1,aes(gs,out_cost, color = jmax, group = jmax), linetype =4)+
...
ggplot()+
  geom_line(data = df_res2, aes(-psi_soil+dpsi,out_a, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res2,aes( -psi_soil+dpsi,out_cost, color = jmax, group = jmax), linetype =4)+
...
ggplot()+
  geom_line(data = df_res2, aes(gs,out_a, color = jmax, group = jmax), linetype =2, show.legend = FALSE)+
  geom_line(data = df_res2,aes(gs,out_cost, color = jmax, group = jmax), linetype =4)+
...
par %>% 
  split(seq(nrow(.))) %>% 
  purrr::map_df(function(x){
    fn_profit_phydro_wue_psi_soil(x,
                 par_cost = par_cost, 
                 par_photosynth = par_photosynth, 
                 par_plant = par_plant, 
                 par_env = par_env, 
...
dat_soilm_numerical = tibble(var=seq(p_crit,0,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_wue_test(tc = 25, ppfd = 400, vpd = 1000, co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = .,  par_plant = list(conductivity = 1e-16,psi50 = -1.46, b=1.4), rdark = 0.02, par_cost = list(alpha=0.11,gamma = 120))),
