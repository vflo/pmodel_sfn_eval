---
title: "The P Hydro model with cmax optimization"
author: "Victor Flo"
date: "`r Sys.Date()`"
output:
  html_document: null
...
The cmax stomatal scheme is obtimised as:
$$
F = A_j - \gamma\,\mathrm{\psi_l}^2 - \omega\,\mathrm{\psi_l}
$$

Then, accounting for photosynthetic cost:
$$
F = A_j - \gamma\,\mathrm{\psi_l}^2 - \omega\,\mathrm{\psi_l} - \alpha J_\mathrm{max}
$$
...
fn_profit_phydro_sox_psi_soil <- function(par, e_crit, p_crit, par_cost, par_photosynth, 
  ...
  vcmax = calc_vcmax_coordinated_numerical(a,ci, par_photosynth)
    risk = par_cost$gamma*psi_leaf^2 -1*psi_leaf
...
fn_profit_phydro_cmax <- function(par, psi_soil, par_cost, par_photosynth, par_plant, par_env, do_optim = FALSE, opt_hypothesis){
...
fn_profit_phydro_cmax_test <- function(par, psi_soil, par_cost, p_crit, e_crit,
...
calc_phydro_cmax_test <- function(tc, ppfd, vpd, co2, elv, LAI, fapar, kphio, psi_soil, rdark, par_plant,  par_cost = NULL){
  ...
  jmax_lim = exp(7)
    # 3. Optimizer
  lj_dps = optimise_stomata_phydro_test(fn_profit_phydro_cmax_test, 
                                     psi_soil = psi_soil, 
                                     e_crit = e_crit,
                                     p_crit= p_crit,
                                     par_cost  = par_cost, 
...
$$
F = g_\mathrm{s}  c_\mathrm{a} (1-\chi) - \gamma\,(\mathrm{\psi_s-\Delta\psi})^2 - \omega\,(\mathrm{\psi_s-\Delta\psi}) - \alpha J_\mathrm{max}
$$

Solved by $\chi$
$$
\frac{\partial F}{\partial \chi} = -\alpha\frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \chi} -g_s\,c_a = 0
$$

Solved by $\Delta\psi$
$$
\frac{\partial F}{\partial \Delta\psi} = - \alpha\,\frac{\partial J_\mathrm{max}}{\partial A_{Jm}} \frac{\partial A_{Jm}}{\partial \Delta\psi}- \frac{\partial g_s}{\partial \Delta\psi}\,c_a\,(1-\chi) - 2\,\gamma\,(\mathrm{\psi_s-\Delta\psi})-\omega=0
$$
...
par %>% 
  split(seq(nrow(.))) %>% 
  purrr::map_df(function(x){
    fn_profit_phydro_cmax_psi_soil(x,
                 par_cost = par_cost, 
                 par_photosynth = par_photosynth, 
                 par_plant = par_plant, 
                 par_env = par_env, 
...
#   mutate(out_hydraulics = purrr::map(var, ~phydro_cmax_bf(.)) ) %>%
#   unnest_wider(out_hydraulics, names_sep = "_")
dat_soilm_numerical = tibble(var=seq(-6,0,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_cmax_test(tc = 25, ppfd = 400, vpd = 1000, co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = .,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")
...
dat_vpd_numerical = tibble(var=seq(100,3000,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_cmax_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -1,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")

dat_vpd2_numerical = tibble(var=seq(100,3000,length.out=20)) %>% 
  mutate(out_hydraulics = purrr::map(var, ~calc_phydro_cmax_test(tc = 25, ppfd = 400, vpd = ., co2 = 400, elv = 0,LAI = 1.5, fapar = 0.7, kphio = 0.087, psi_soil = -3,  par_plant = par_plant, rdark = 0.02, par_cost = list(alpha=0.1)))) %>%
  unnest_wider(out_hydraulics, names_sep = "_")
