source("init_SFN_MODEL_PREDICTION_HOURLY.R")
...
as.list(list_files) %>%
  purrr::map(function(x){
    ...
    sfn_aggr_hour <- calc_sfn_aggr_E_hourly(sfn_agr,"1 hour")
    env_hour <- calc_sfn_aggr_env_hourly(sfn,"1 hour")
    ...
    #### PMODEL WEEKLY ####
    #DATA preparation
    df <- data_prep_hourly(sfn_aggr_hour, env_hour, opt_swc) %>% bind_cols(si_code = si_code)
    ...
    ## PMODEL HOURLY ##
    print("Calculating PMODEL SUBDAILY")
    pmodel_subdaily <- calc_pmodel_subdaily(df, soil, par_plant_std)%>% as_tibble()
